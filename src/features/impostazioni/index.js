...
// ===== Utilizzo Firestore (stima) – visibile solo Supervisor =====
    ...
    async initReleaseChangelog() {
      ...
      try {
        ...
        if (changesEl) {
          changesEl.innerHTML = (current.changes || []).map(x => `<li>${x}</li>`).join('') || '<li>Nessuna nota disponibile</li>';
        }
        if (histEl) {
            ...
            <div class="accordion-item">
              <h2 class="accordion-header" id="rel-h-${idx}">
                <button class="accordion-button ${idx === 0 ? '' : 'collapsed'}" type="button" data-bs-toggle="collapse" data-bs-target="#rel-c-${idx}" aria-expanded="${idx===0?'true':'false'}">
                  ${item.version || '-'} - ${item.date || ''} - ${item.name || ''}
                </button>
              </h2>
        ...
        if (changesEl) changesEl.innerHTML = `<li>${e?.message || e}</li>`;
